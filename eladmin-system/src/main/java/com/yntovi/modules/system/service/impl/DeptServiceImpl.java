...
import com.yntovi.modules.system.service.RolesDeptsService;
import com.yntovi.modules.system.service.dto.DeptDto;
import com.yntovi.modules.system.service.dto.DeptQueryParam;
import com.yntovi.modules.system.service.mapper.DeptMapper;
import com.yntovi.modules.system.service.mapper.RoleMapper;
...
import com.yntovi.utils.SecurityUtils;
import com.yntovi.utils.enums.DataScopeEnum;
...
public class DeptServiceImpl extends CommonServiceImpl<DeptMapper, Dept> implements DeptService {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public boolean deptSync(JSONObject obj) {
        JSONArray jsonArray = JSONUtil.parseArray(obj.get("department"));
        if (ObjectUtil.isNull(jsonArray)) return true;
        for (Object o : jsonArray) {
            JSONObject parseObj = JSONUtil.parseObj(o);
            Long id = Long.parseLong(parseObj.get("id").toString());
            Long parentid = Long.parseLong(parseObj.get("parentid").toString());

            QueryWrapper<Dept> queryWrapper = new QueryWrapper<>();
            queryWrapper.eq("wx_dept_id", id);
            Dept dept1 = deptMapper.selectOne(queryWrapper);// 根据  企业微信部门id 查询
            Dept dept2=null;
            if (!new Long(0).equals(parentid)) {
                QueryWrapper<Dept> queryWrapper2 = new QueryWrapper<>();
                queryWrapper2.eq("wx_dept_id", parentid);
                 dept2 = deptMapper.selectOne(queryWrapper2); // 根据企业微信部门parentid 查询在数据库中的ID
            }

            if (ObjectUtil.isNull(dept1)) {
                // 添加
                Dept dept = new Dept();
                dept.setName(parseObj.get("name").toString()); // 部门名
                dept.setWxDeptId(id); // 部门id
                if(ObjectUtil.isNotNull(dept2)) {
                    dept.setPid(dept2.getId());  // 部门父id = 根据企业微信父id查询出来的id
                }
