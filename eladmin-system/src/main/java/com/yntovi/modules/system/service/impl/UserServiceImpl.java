...
import com.yntovi.modules.system.service.mapper.DeptMapper;
...
import com.yntovi.modules.system.service.mapper.UsersJobsMapper;
import com.yntovi.modules.system.service.mapper.UsersRolesMapper;
import com.yntovi.modules.system.service.*;
import com.yntovi.modules.system.service.dto.DeptDto;
import com.yntovi.modules.system.service.dto.UserDto;
import com.yntovi.modules.system.service.dto.UserQueryParam;
import com.yntovi.modules.security.service.OnlineUserService;
import com.yntovi.modules.security.service.UserCacheClean;
import com.yntovi.modules.system.service.dto.DeptSmallDto;
import com.yntovi.modules.system.service.dto.JobSmallDto;
import com.yntovi.modules.system.service.dto.RoleSmallDto;
import com.yntovi.modules.system.service.mapper.UserMapper;
...
public class UserServiceImpl extends CommonServiceImpl<UserMapper, User> implements UserService {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public boolean userSync(JSONObject obj) throws Exception {
        JSONArray jsonArray = JSONUtil.parseArray(obj.get("userlist"));  // 获取用户列表
        if (ObjectUtil.isNull(jsonArray)) return true;
        for (Object o : jsonArray) {
            JSONObject parseObj = JSONUtil.parseObj(o);
            //查找用户
            QueryWrapper<User> userQueryWrapper = new QueryWrapper<>();
            userQueryWrapper.eq("username", parseObj.get("userid").toString());
            User user = userMapper.selectOne(userQueryWrapper);

            Long department = Long.valueOf(parseObj.get("main_department").toString()); // 查询部门
            QueryWrapper<Dept> deptQueryWrapper = new QueryWrapper<>();
            deptQueryWrapper.eq("wx_dept_id", department);
            Dept dept = deptMapper.selectOne(deptQueryWrapper);

            UserDto userDto = new UserDto();
            userDto.setUsername(parseObj.get("userid").toString()); //账号
            userDto.setNickName(parseObj.get("name").toString());  //昵称
            userDto.setIsAdmin(false); // 是否是超级管理员
            userDto.setPhone(parseObj.get("mobile").toString()); //手机号
            userDto.setDeptId(ObjectUtil.isNotNull(dept) ? dept.getId() : null); // 部门id
            userDto.setGender(parseObj.get("gender").toString().equals("1") ? "男" : "女");
            userDto.setPassword(passwordEncoder.encode("123456"));
            if (ObjectUtil.isNull(user)) {
//                添加
                userDto.setEnabled(true);  // 是否激活
                RoleSmallDto roleSmallDto = new RoleSmallDto();
                roleSmallDto.setId(2L);
...
//                修改
    ...
    public boolean save1(UserDto resources) {
        User user = ConvertUtil.convert(resources, User.class);
        int ret = userMapper.insert(user);
        final Long userId = user.getId();
        if (CollectionUtils.isNotEmpty(resources.getRoles())) {
            resources.getRoles().forEach(role -> {
    ...
    public boolean updateById1(UserDto resources) throws Exception {
        User user = ConvertUtil.convert(resources, User.class);
        return userMapper.updateById(user) > 0;
    }
